...
class MyTrainer(Trainer):
    ...
    def _maybe_log_save_evaluate(
        self, tr_loss, grad_norm, model, trial, epoch, ignore_keys_for_eval, **kwargs
    ):
        if (
            ...
            for k in kwargs:
                if "_loss" in k:
                    other_loss_scalar = self._nested_gather(kwargs[k]).mean().item()
                    kwargs[k] -= kwargs[k]
                    logs[k] = round(
                        other_loss_scalar
                        / (self.state.global_step - self._globalstep_last_logged),
    ...
    def training_step(
        ...
        if return_outputs:
            for k in outputs:
                if "_loss" in k:
                    outputs[k] = (
                        outputs[k].detach() / self.args.gradient_accumulation_steps
                    )
    ...
    def _inner_training_loop(
        ...
        # added
        reppred_loss = torch.tensor(0.0).to(args.device)
        aedec_loss = torch.tensor(0.0).to(args.device)
        sae_spar_loss = torch.tensor(0.0).to(args.device)
        sae_rec_loss = torch.tensor(0.0).to(args.device)
        ...
        # -- momentum schedule
        if (
            hasattr(model.module.model_args, "ema_m1")
            and model.module.model_args.ema_m1 > 0
        ):
            ema = [model.module.model_args.ema_m1, model.module.model_args.ema_m2]
            momentum_scheduler = (
                ema[0] + i * (ema[1] - ema[0]) / (max_steps)
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                with self.accelerator.accumulate(model):
                    # added
                    tr_loss_step, loss_outputs = self.training_step(
                        model, inputs, return_outputs=True
                    )
                    reppred_loss_step = loss_outputs["reppred_loss"]
                    aedec_loss_step = loss_outputs["aedec_loss"]
                    sae_spar_loss_step = loss_outputs["sae_spar_loss"]
                    sae_rec_loss_step = loss_outputs["sae_rec_loss"]
                ...
                if (
                    ...
                    # added
                    reppred_loss += reppred_loss_step
                    aedec_loss += aedec_loss_step
                    sae_spar_loss += sae_spar_loss_step
                    sae_rec_loss += sae_rec_loss_step
                ...
                if (
                    ...
                    if (
                        ...
                        model._momentum_update_encoder(m=momentum)
